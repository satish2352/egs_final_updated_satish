                                ...
                                <ul class="pagination">
                                    <li class="{{ ($register_user->currentPage() == 1) ? ' disabled' : '' }}">
                                        @if ($register_user->currentPage() > 1)
                                            <a href="{{ $register_user->appends(['search' => request()->input('search')])->url($register_user->currentPage() - 1) }}">Previous</a>
                                        @else
                                            <span>Previous</span>
                                        @endif
                                    </li>
                                    ...
                                    @if ($startPage > 1)
                                        <li>
                                            <a href="{{ $register_user->appends(['search' => request()->input('search')])->url(1) }}">1</a>
                                        </li>
                                        @if ($startPage > 2)
                                            <li>
                                                <span>...</span>
                                    ...
                                    @for ($i = $startPage; $i <= $endPage; $i++)
                                        <li class="{{ ($currentPage == $i) ? ' active' : '' }}">
                                            <a href="{{ $register_user->appends(['search' => request()->input('search')])->url($i) }}">{{ $i }}</a>
                                        </li>
                                    @endfor
                                    @if ($endPage < $lastPage)
                                        ...
                                        @endif
                                        <li>
                                            <a href="{{ $register_user->appends(['search' => request()->input('search')])->url($lastPage) }}">{{ $lastPage }}</a>
                                        </li>
                                    @endif
                                    <li class="{{ ($currentPage == $lastPage) ? ' disabled' : '' }}">
                                        @if ($currentPage < $lastPage)
                                            <a href="{{ $register_user->appends(['search' => request()->input('search')])->url($currentPage + 1) }}">Next</a>
                                        @else
                                            <span>Next</span>
                                        @endif
                                    </li>
